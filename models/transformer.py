...
class ContextEncoder(nn.Module):
    def __init__(self, input_dim_text=256, input_dim_image=256,device="cuda", *args, **kwargs):
        super(ContextEncoder, self).__init__(*args, **kwargs)
        # Attributes
        self.context_text_dim = input_dim_text
        self.context_image_dim = input_dim_image
        self.device = device
        ...
        self.image_context = nn.Parameter(torch.normal(mean=0, std=image_variance.item(),
                                                       size=(1, self.context_image_dim)).to(self.device))

        self.text_context = nn.Parameter(torch.normal(mean=0, std=text_variance.item(),
                                                      size=(1, self.context_text_dim)).to(self.device))
    ...
    def forward(self, inputs):
        ...
        # context = torch.where(torch.tensor(x == "image"), self.image_context, self.text_context)
...
class UniversalEncoder(nn.Module):
    ...
    def __init__(self, dim = 256, num_head=8, num_layers=4, *args, **kwargs):
        super(UniversalEncoder, self).__init__(*args, **kwargs)
        self.dim = dim
        ...
        self.layer_normalization = nn.LayerNorm(self.dim)

    def forward(self, embeddings, type_input):
        # embeddings size : (L, N, D) -> (N, L, D)
        embeddings = embeddings.permute(1, 0, 2)
        if type_input == "image":
            contexts = self.context_encoder("image").repeat(embeddings.size(0), 1)
            fusion = embeddings + contexts.unsqueeze(1)
            norm = self.layer_normalization(fusion)
            representation = self.transformer_encoder(norm)
            residual_connection = representation + fusion

        else:
            contexts = self.context_encoder("text").repeat(embeddings.size(0), 1)
            fusion = embeddings + contexts.unsqueeze(1)
            norm = self.layer_normalization(fusion)
            representation = self.transformer_encoder(norm)
            residual_connection = representation + fusion

        return residual_connection.permute(1,0,2)


    def __call__(self, inputs, type_input="image"):
        return self.forward(inputs, type_input)
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.universal_encoder = UniversalEncoder()
    ...
    def forward(
        ...
        if encode_and_save:
            # flatten NxCxHxW -> NxCxHW -> to NxHWxC
            bs, c, h, w = src.shape

            #src = self.universal_encoder(src, "image")
            # HWxNxC
            ...
            if isinstance(text[0], str):
                ...
                ########################################################################################################
                #encoded_text.last_hidden_state = self.universal_encoder(encoded_text.last_hidden_state, "text")
            ...
            # Fusion
            src = self.universal_encoder(src, "image")
            text_memory_mask = self.universal_encoder(text_memory_resized, "text")
